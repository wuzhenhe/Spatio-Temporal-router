...
from data import _build_messages, _load_raw_dataset, _normalize_choice_pairs, _render_messages
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate CommonsenseQA answers and compute multiple-choice accuracy.")
    ...
    parser.add_argument("--output-dir", default="./eval_outputs/commonsenseqa_accuracy", help="Output directory.")
...
def normalize_choice_label(answer: Any, choice_pairs: List[Tuple[str, str]]) -> Optional[str]:
...
def extract_commonsenseqa_answer(text: str, choice_pairs: List[Tuple[str, str]]) -> Optional[str]:
    if text is None:
        return None
    labels = [label for label, _ in choice_pairs]
    ...
    explicit_patterns = [
        ...
        rf"(?:answer|option|choice|final answer)\s*(?:is|:)?\s*([{label_pattern}])\b",
    ...
    normalized_text = normalize_choice_text(stripped)
...
def format_choices_for_prediction(choice_pairs: List[Tuple[str, str]]) -> List[Dict[str, str]]:
    return [{"label": label, "text": text} for label, text in choice_pairs]
...
def main() -> None:
    ...
    for stale_tmp in output_dir.glob(f"{local_metrics_output.name}.tmp.*"):
        stale_tmp.unlink()
    print(
        f"[rank{rank}] Starting CommonsenseQA generation: local_samples={len(local_examples)}, "
    ...
    for local_pos, (idx, example) in enumerate(local_examples, start=1):
        ...
        ref_answer = normalize_choice_label(example.get("answerKey", example.get("answer")), choice_pairs)
        pred_answer = extract_commonsenseqa_answer(generated_text, choice_pairs)
        exact_match = pred_answer is not None and ref_answer is not None and pred_answer == ref_answer
        prediction = {
            "index": idx,
            "id": example.get("id"),
            "question": example["question"],
            "question_concept": example.get("question_concept"),
            "choices": format_choices_for_prediction(choice_pairs),
